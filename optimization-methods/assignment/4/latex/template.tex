...
whose \( A \) matrix is symmetric and \textbf{positive definite}.
...
the current eigval and the largest as the formula suggests:
\( \norm{x_{k+1} - x^*}_A^2
\leq
...
Before explaining the Simplex method, let's first introduce the concept of linear programming.
...
The feasible set is \textbf{polytope}: a convex, connected set with flat, polygonal faces.
...
\noindent where \( c, x \in \mathbb{R}^n, \; b \in \mathbb{R}^m, \; A \in \mathbb{R}^{m \times n}\).
...
\noindent we can convert the inequality constraints to equalities by introducing a vector of
...
\noindent It's not yet in standard form
...
\noindent The problem (\ref{eq:ex3-example-f-constraint-slack}) can be written as:
...
\noindent which clearly has the same form as (\ref{eq:ex3-linear-program}).
...
\noindent The optimality conditions for problem (\ref{eq:ex3-linear-program})
are only the first-order alias KKT conditions.
The convexity of the problem ensures that these conditions are sufficient for a global minimum.
The KKT conditions requires linear independence of the active constraint gradients (LICQ).
...
into two vectors \( \lambda \in \mathbb{R}^m \) multiplier vector for equality constraints \( Ax = b \),
and \( s \in \mathbb{R}^n \) multiplier vector for the bound constraints \( x \geq 0 \).
The Lagrangian function for the problem (\ref{eq:ex3-linear-program}) is:

\myex{true}{eq:ex3-linear-lagrangian}{
    \mathcal{L}(x, \lambda, s) = c^T x + \lambda^T (Ax - b) - s^T x
}

\noindent Applying Theorem (\ref{eq:ex2-kkt}), we find that the first-order necessary conditions for
...
\noindent Let \( \vibar{x}{1} := x_1 \geq 0 \; \cap \; x_2 \geq 0 \),
...
\noindent Let \( \vibar{x}{2} := x_1 \geq 0 \; \cap \; 3 + 3 x_1 - 2 x_2 \geq 0 \).
...
\noindent Let \( \vibar{x}{3} := 3 + 3 x_1 - 2 x_2 \geq 0 \; \cap \; 6 - 2 x_1 - 3 x_2 \geq 0 \).
The intersection is computed by solving the system:
...
\noindent Let \( \vibar{x}{4} := 6 - 2 x_1 - 3 x_2 \geq 0 \; \cap \; 4 - 2 x_1 - x_2 \geq 0 \).
The intersection is computed by solving the system:
...
\noindent Let \( \vibar{x}{5} :=  4 - 2 x_1 - x_2 \geq 0 \; \cap \; x_2 \geq 0 \).
The intersection is computed by solving the system:
...
\noindent Find \( \lambda_5, \lambda_6 \):
...
\noindent Finally, let's check that the gradient of the Lagrangian function is zero at the optimal point
\( \mathbf{x}^* = \vibar{x}{1} \),
...
\noindent Hence, the first order necessary conditions hold for the optimal point \( \vibar{x}{1} \).
