...
we will assume throughout that \( m \le n \) for simplicity.
...
\noindent As \( B \) is the basic matrix, whose columns are \( A_i \) for \( i \in \mathcal{B} \),
...
\noindent From the KKT conditions (\ref{eq:ex3-linear-kkt}), we have:

\myex{true}{eq:ex3-deriving-kkt-partition}{
    Ax &= B x_B + N x_N = b. \\
}

\noindent The primal variable \( x \) for this simplex iterate is defined as:
