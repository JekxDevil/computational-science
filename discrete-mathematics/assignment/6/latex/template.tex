...
This means that \textbf{at most} 1 of \( (f(0), f(1), \dots, f(m \cdot l)) \) can be 1, 
\( \forall m \in \N \) in order for all such function to belong to \( A_l \).
...
\( m \cdot l \) sized input.
\\

From Theorem 3.23, the set \( \semiinfbinseq \) is uncountable.
Building an injection from \( \semiinfbinseq \) to such above construction econding \( f \) functions 
in \( A_l \) is proposed.
Let's define the function \( g \):
%  \( g: \semiinfbinseq \to A_l \) with \( \alpha \in \semiinfbinseq, \) 
\myex{false}{ex6-5-g}{
...
every possible semi infinite binary sequence has a valid function \( f \) mapping in \( A_l \).
\\
