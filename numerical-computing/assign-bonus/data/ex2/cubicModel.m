...
model_cubic = @(a1, a2, a3, a4, x) a1 + a2*x + a3*x.^2 + a4*x.^3;
[n, m] = size(data_oil);
A_oil = ones(n, 4);
A_oil(:,2) = data_oil(:,1);
A_oil(:,3) = data_oil(:,1) .* data_oil(:,1);
A_oil(:,4) = data_oil(:,1) .* data_oil(:,1) .* data_oil(:,1);
b_oil = data_oil(:,2);
...
A_kero(:,4) = data_kerosene(:,1) .* data_kerosene(:,1) .* data_kerosene(:,1);
b_kero = data_kerosene(:,2);
[x_kero, euclidean_norm_kero, SE_kero, RMSE_kero] = leastSquares(A_kero, b_kero);

% visualize data results
figure;
plot(data_oil(:,1), model_cubic(x_oil(1), x_oil(2), x_oil(3), x_oil(4), data_oil(:,1)), "red");
hold on;
scatter(data_oil(:,1), data_oil(:,2), "black");
hold on;
plot(data_oil(:,1), data_oil(:,2), "blue");
title("crude oil dataset - cubic model")
legend("cubic least square", "discrete real data", "real data trend");
hold off;

figure;
plot(data_kerosene(:,1), model_cubic(x_kero(1), x_kero(2), x_kero(3), x_kero(4), data_kerosene(:,1)), "red");
hold on;
scatter(data_kerosene(:,1), data_kerosene(:,2), "black");
hold on;
plot(data_kerosene(:,1), data_kerosene(:,2), "blue");
title("kerosene dataset - cubic model")
legend("cubic least square", "discrete real data", "real data trend");
hold off;

fprintf("\t\t Oil \t kerosene\n");
fprintf("Cubic: %f \t %f \n", ...
    model_cubic(x_oil(1), x_oil(2), x_oil(3), x_oil(4), [2012]), ...
    model_cubic(x_kero(1), x_kero(2), x_kero(3), x_kero(4), [2012]));